// Thin OpenRouter chat client. The model id comes from config (env) and is
// never returned to callers or logged, so the bot stays model-agnostic and the
// underlying model remains secret.
...
export class LLM {
  ...
  async chat(messages, opts = {}) {
    ...
    // Ask for JSON when the caller drives a structured protocol. Many free
    // models reject response_format with a 4xx, so we degrade gracefully: try
    // with it, and on a client error retry once without it. The agent loop
    // parses leniently either way.
    let data;
    try {
